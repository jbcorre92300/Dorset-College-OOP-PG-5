...
namespace oop_group5_project
{
    class Teacher
    {
        ...
        public void AddaClassforaClassroom(string name, int classname, Date date, string location)
        {
            //int i = l'index de la classe dans la liste de class du teacher
            int i = 1;
            Teacher teacher = new Teacher(name, listclassroom, matter);
            Class newclass = new Class(name,date, matter, location, teacher);
            foreach(Student student in listclassroom[i].studentlist)
            {
